...
vector before decoding.  This module keeps the 7x7 spatial grid, lets the
caption decoder attend to a different region at every word step, and exposes
those attention weights for visualisation.
"""
...
class SpatialCNNEncoder(nn.Module):
    ...
    def __init__(self, encoder_dim: int = config.ATTENTION_ENCODER_DIM, fine_tune: bool = False):
        super().__init__()
        try:
            resnet = tv_models.resnet50(weights=tv_models.ResNet50_Weights.IMAGENET1K_V1)
        except Exception:
            resnet = tv_models.resnet50(weights=None)
            print("Warning: ImageNet weights unavailable; ResNet50 was randomly initialised.")

        # layer4 output: (B, 2048, 7, 7) for 224x224 input.
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        features = self.projection(features)  # (B, D, H, W)
        # Decoder consumes (B, locations, D).
...
class AttentionLSTMDecoder(nn.Module):
    """Word decoder whose recurrent state selects a visual region each step."""
    ...
    def _init_glove(self, glove_matrix: np.ndarray) -> None:
        ...
        if tensor.shape[1] == self.embed_dim:
            ...
            # Fixed random projection preserves the external embedding signal
            # without adding a train-time dependency on the GloVe dimensionality.
    ...
    def step(
        ...
        """Decode one token and return logits, recurrent state and attention."""
        hidden, cell = state
    ...
    def forward(self, encoder_out: torch.Tensor, captions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Teacher-forcing pass returning word logits and per-step attention maps."""
        ...
        for t in range(steps):
            step_logits, state, alpha = self.step(captions[:, t], encoder_out, state)
...
class ExplainableCaptioningModel(nn.Module):
    ...
    architecture_name = "attention_lstm"
    ...
    def __init__(
        ...
        super().__init__()
        self.encoder = SpatialCNNEncoder(encoder_dim=encoder_dim, fine_tune=fine_tune_cnn)
    ...
    def parameter_groups(self, lr: float, cnn_lr_factor: float) -> list[dict]:
        ...
        other = [p for p in self.parameters() if id(p) not in cnn_ids and p.requires_grad]
        return [
            {"params": other, "lr": lr},
            {"params": [p for p in cnn_params if p.requires_grad], "lr": lr * cnn_lr_factor},
        ]


def attention_coverage_loss(alphas: torch.Tensor) -> torch.Tensor:
    """Encourage the decoder to distribute attention over the visual grid.

    This is the doubly-stochastic regularisation idea from *Show, Attend and
    Tell*: across the caption, each image region should receive some attention.
    """
